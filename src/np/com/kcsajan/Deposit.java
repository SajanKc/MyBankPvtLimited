...
public class Deposit extends JInternalFrame implements ActionListener {

	private static final long serialVersionUID = 1L;

	private JPanel jpDeposit = new JPanel();
	...
	public Deposit() {
		...
		super("Deposit", false, true, false, true);
		setSize(350, 300);

		jpDeposit.setBounds(0, 0, 500, 115);
		jpDeposit.setLayout(null);
		...
		jpDeposit.add(lbAccountNumber);
		jpDeposit.add(textAccountNumber);
		jpDeposit.add(btnVerifyAccount);

		jpDeposit.add(lbFullName);
		jpDeposit.add(textFullName);

		jpDeposit.add(lbRegisteredDate);
		jpDeposit.add(textRegisteredDate);

		jpDeposit.add(lbBalance);
		jpDeposit.add(textBalance);

		jpDeposit.add(btnSave);
		jpDeposit.add(btnCancel);

		// Adding Panel to Window.
		getContentPane().add(jpDeposit);

		// Showing the New Account Window.
		setFrameIcon(new ImageIcon("images\\deposit.png"));
		setVisible(true);
		setLocation(20, 20);
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == btnSave) {
			...
			if (textAccountNumber.getText().equals("")) {
				...
				Customer.DepositMoney(con, accountNumber, balance);
				textBalance.setText("");
			}
		}
		if (obj == btnCancel) {
